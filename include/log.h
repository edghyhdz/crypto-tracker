...
std::string current_time() {
    ...
    strftime(time_str, 256, date_time_format, localtime(&time));
...
class Log {
   public:
    ...
    std::ostringstream &get() {
        _stream << current_time() << " "
                << level_strings[static_cast<int>(_level)] << " ("
                << std::this_thread::get_id() << ")\t"
                << _file.filename().string() << ":" << _line << ":\t";
        return _stream;
    }

    std::ostringstream &get(std::string &&message) {
        std::ofstream logFile("crypto_tracker.log", std::ios_base::app);
        _stream << current_time() << " "
                << level_strings[static_cast<int>(_level)] << " ("
                << std::this_thread::get_id() << ")\t"
